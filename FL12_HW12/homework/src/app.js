...
if (localStorage.getItem('readyList')) {
    readyList = JSON.parse(localStorage.getItem('readyList'));
}
...
function createSetList() {
    ...
    const createItem = (set, status = null) => {
        ...
        removeBtn.addEventListener('click', (event) => {
            ...
            readyList = readyList.filter(item => item.id !== set.id);
            saveToStorage();
            goToPage('main');
        });
...
function getNewId() {
    ...
    id = readyList.reduce((max, item) => item.id > max ? item.id : max, id);
...
function saveToStorage() {
    ...
    localStorage.setItem('readyList', JSON.stringify(readyList));
}
